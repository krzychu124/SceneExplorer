...
namespace SceneExplorer.ToBeReplaced.Windows
{
    public abstract class FloatingWindowBase : MonoBehaviour
    {
        ...
        private float _clickInterval = 0.20f;
        private float _lastMinimizeClickTime;
        ...
        if (WindowManager.FocusedWindowId == -1 || (_applyAction.WasReleasedThisFrame() && CursorOverUI))
        {
        ...
        if (!GUIUtility.hasModalWindow)
        {
            ...
            {    
                moveTex = Style.TitleHoverTexture;
                if (_applyAction.IsPressed() && _resizingWindow == null)
                {
                    _movingWindow = this;
        ...
        Texture2D texture = IsRoot ? Style.RootColorTexture : (_resizingWindow == this || _movingWindow == this) ? Style.InteractionColorTexture : Style.TitleNormalTexture;
