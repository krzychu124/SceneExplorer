...
namespace SceneExplorer.ToBeReplaced.Windows
{
    public class PrefabDataInspector : FloatingWindowBase, IParentInspector, IClosablePopup, IValueInspector
    {
        ...
        public PrefabBase SelectedPrefab
        {
            ...
            set
        {
            Logging.Info("Selecting PrefabData");
        ...
        if (_dataChanged)
        {
            ...
            if (_selectedPrefab)
            {
                ...
                BuildComponentsUI();
                Logging.Info($"Generated {_components.Count} components");
            }
        ...
        Logging.Info($"Prefab: {_prefabTypeName}");
        ObjectItem.PrepareItems(_selectedPrefab.GetType(), _selectedPrefab, _prefabTypeName, _components);
        Logging.Info($"Prepared {_components.Count} items");
        
        foreach (ISectionItem sectionItem in _components)
        {
            if (sectionItem is IInteractiveSectionItem item)
            {
                item.ParentInspector = this;
            }
        }
