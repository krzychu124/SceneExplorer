...
namespace SceneExplorer.ToBeReplaced.Helpers.ContentItems
{
    public class ObjectItem : ISectionItem
    {
        ...
        if (CommonUI.CollapsibleHeader($"{_title} ", _expanded, out bool _, location: CommonUI.ButtonLocation.AfterTitle, textStyle: !_expanded ? UIStyle.Instance.reducedPaddingLabelStyle : UIStyle.Instance.reducedPaddingHighlightedLabelStyle))
        {
            _expanded = !_expanded;
        }
        GUI.enabled = true;
        ...
        public static void PrepareItems(Type type, object value, string sectionName, List<ISectionItem> items) {
        foreach (FieldInfo fieldInfo in type.GetRuntimeFields())
        {
            ...
            items.Add(UIGenerator.GetSectionItem(fieldInfo.Name, fieldInfo.GetValue(value), sectionName));
        }
    }
    }
}
